...
module.exports.addLivescoresDetailsCustom = async function (format) {
  ...
  console.log(matches?.length, format, 'matchestzr')
  // await delay(10)
  ...
  for (let i = 0; i < matches.length; i++) {
    try {
      ...
      //console.log(match,'much')
      if ((!match) || match?.result == "Complete" || (!match?.isInPlay)) {
        console.log(match?.matchId, i, 'matchId')
        continue;
      } else {
        console.log('abd')
        const keys = await getkeys(matchId);
        console.log(keys, 'jeys');
   ...
   headers: { 
      ...
      'x-apihub-endpoint': '5f260335-c228-4005-9eec-318200ca48d6'
        ...
        await delay(3000); // Add a delay of 1 second between requests
        try {
          ...
          if (usageCount >= 100) {
            console.log("usage count 100", usageCount)
            const result = await RapidApiKey.updateOne(
              { apiKey: keys },
              { $set: { status: 'inactive', usageCount: usageCount } }
            );
            let k = await RapidApiKey.findOne({ apiKey: keys })
            ...
            if (nextKey) {
              ...
              console.log("Switched to new API tkey:", nextKey.apiKey);
            } else {
